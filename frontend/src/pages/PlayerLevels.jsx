...
import { MAX_COMBAT_LEVEL, MAX_FORAGING_LEVEL } from '../lib/playerStats';
import { SLOT_TEXTURES } from '../lib/icons';

const panel =
  'bg-[#c6c6c6] border-[3px] border-t-white border-l-white border-b-[#555555] border-r-[#555555] outline outline-2 outline-black';

// Combat/Skyblock/Foraging Level — grouped on one small edit page behind
// the grid slot above Accessories (see Landing.jsx), same "click a slot,
// edit on its own page" pattern as Stat Tuning. Combat and Skyblock Level
// behave exactly as before (see lib/playerStats.js); Foraging Level is
// new here and only feeds a flat Strength bonus into Damage Sources'
// (base) Stats — it has no slot of its own to click through to.
export default function PlayerLevels() {
  const navigate = useNavigate();
  const { playerStats, setCombatLevel, setSkyblockLevel, setForagingLevel } = useBuild();
