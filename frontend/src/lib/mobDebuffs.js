...
// player. All three are Catacombs mechanics, so the panel they live on is Dungeon-gated the same
// way the Blessings panel is (user-specified 2026-09-14).
//
// Two of the three reduce the mob's Defense STAT, not the damage: lib/mobDefenses.js turns Defense
// into a multiplier as `1 - Def/(100+Def)`, so a Defense cut feeds that formula rather than
// multiplying the result. That distinction matters — halving Defense is not "+100% damage", and on
// Master Necron (2100) it is worth considerably more than halving would suggest.
//
...
// there is no published Defense number to cut. Ice Spray is the only one of the three that does
...
export function emptyDebuffs() {
  return { iceSpray: false, lastBreath: 0, lethality: 0 };
}
...
// A flat external multiplier on final damage, applied at the same last step as the mob's own
// Defense/Damage Reduction multipliers — see lib/finalDamage.js.
export function iceSprayMultiplier(debuffs) {
  return debuffs?.iceSpray ? ICE_SPRAY_MULTIPLIER : 1;
}
...
export function hasAnyDebuff(debuffs) {
  return !!debuffs?.iceSpray || lastBreathLevel(debuffs) > 0 || lethalityStacks(debuffs) > 0;
}
